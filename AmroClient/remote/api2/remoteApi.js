...
$(document).ready(function() {
	...
	$("#new_certificate_button").click(function(){
		newCertificate();
	});
	
	$("#delete_certificate_button").click(function(){
		deleteCertificate();
	});
	
	$("#get_certificate_button").click(function(){
		getCertificate();
	});
	...
	function newCertificate()
	{
		$.getJSON("http://localhost:8080/?callback=?",
		{
			...
			protn: "a1",
			ordencompra: "7788",
			observations: "Certificado Prueba",
			description: "CUERPO FILTRO Y 450",
			identificacion: "H108",
			termicopath: "/disco/Amro/imagen_especto.png",
			client_id: "5",
			material_id: "5",
			approver_id: "6",
			reviewer_id: "6",
			chemicalanalysis: "c=5+mn=6+si=1.1+p=2.3+s=1.5+cr=4+ni=0+mo=0+cu=0+v=0,1+ce=0",
			mechanicalanalysis: "tension_rotura=1+limite_fluencia=2.2+alargamiento=12+estriccion=5.5+resiliencia=2,1+dureza=0",			
		},
		function(data) {
			html_code += dump(data);
			$("#html_change").replaceWith(html_code);				
